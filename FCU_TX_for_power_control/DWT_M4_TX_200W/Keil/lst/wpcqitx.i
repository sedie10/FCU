...
extern ModulationState modulation_state;                 
extern ModulationType modulation_type;                   
extern int hysteresis_scale_factor;                      

extern void initModulation(void);                        
extern void resetModulation(void);                       
extern int resetHysteresis(void);                        
extern void resetDynamicHysteresis(void);                
extern void advanceModulation(void);                       
...
extern uint8_t g_qi_tx_buffer[32];   
extern uint8_t g_qi_tx_size;                    
...
extern void reset_loop_counters(void);                                 
...
fix16_t qi_coil_current_f16 = 0;                         
fix16_t qi_control_error_value_f16 = 0;                         
uint8_t qi_device_charging = 0;				 
uint8_t qi_device_charged = 0;				 
uint8_t qi_device_recharging = 0;                            
uint8_t qi_device_unknown=0;                                 
uint8_t qi_foreign_object_found = 0;                         
uint8_t qi_comm_enabled = 0;                                 
...
uint8_t efficiency_low_flag = 1;                             
uint8_t dynamic_hysteresis = 0;                              
uint8_t collecting_dynamic_hysteresis = 0;                   
static uint8_t change_modulation_type = 0;                   
uint16_t pwm_duty_ticks = 0;                             
static long delay_counter = 0;

volatile int pwmFreq = 872;


static int i_ring_buffer[4];              


static uint8_t i_ring_buffer_index = 0;                      
...
uint16_t q_adc[81+1]; 
uint32_t q_self_resonance_time_array[81];
...
static int qi_num_rx_optional_packets = 0;				   

int qi_packet_last_bit_timer_ms = 0;               
static int power_xfer_delay_ms = 0; 					   
static int control_error_pkt_ms = 0;					   
static int received_power_pkt_ms = 0;					   
static int modulation_error_pkt_ms = 0; 				   
static int cep_count = 0;
static int charge_terminate_counter = 0;				   
static int num_digital_ping_attempts = 0;				   
...
{ 
    if(g_st_power_contract.count & 0x04)   
        g_st_power_contract.received_power = qi_tx_received_power_header;
    
    if(g_st_power_contract.count & 0x01)   
        g_st_power_contract.guaranteeed_power = (qi_guaranteed_power >> 1) * POWER_OF_TEN[qi_guaranteed_class];
    
    if(g_st_power_contract.count & 0x02)   
        g_st_power_contract.max_power = (qi_maximum_power >> 1) * POWER_OF_TEN[qi_power_class];
    
    if(g_st_power_contract.count & 0x08)   
        g_st_power_contract.modulation_depth = qi_FSKDepth;

		if (1) printf( "\r\n RP:%d GP:%d MP:%d MD:%d",g_st_power_contract . received_power, g_st_power_contract . guaranteeed_power, g_st_power_contract . max_power, g_st_power_contract . modulation_depth);
...
{
    t_control_error_ms = 1500;
    t_received_power_ms = 23000;
}
...
#line 761 "..\\WPCQiTx.c"
	{
		 ...
		 if (1) printf( "coil sel %d\r\n",coil_sel_num);

		

		 if( coil_sel_num == COIL_SEL_2)
			PWM_vUpdateBuckbooster( 960, 960*185/200);
		 else
			PWM_vUpdateBuckbooster( 960, 960*180/200);
		 
		 PWM_vStartBuckbooster();
		 qi_comm_enabled = 1;

		QiTx_vPowerControl(POWER_ON);
		if (change_modulation_type) 
		{
			advanceModulation();
...
WPCQiTx_State_t WPCQiTx_ProcSignalStrength ( void )
{
	...
	if (qi_comm_message_received)
	{
		...
		if (qi_last_packet_header == WPCQi_SignalStrengthPacket_H)
		{
			...
			if (1) printf( "SS received\r\n");

			(*((volatile uint32_t *)(((((uint32_t)0x40000000) + 0x04800UL)+(0x40*(4))) + ((10)<<2)))) =1;
	...
	else
	{
		...
		else
		{
			...
			if (qi_packet_first_bit_timer_ms > 25)
			{
				PWM_vStopCoil();
...
WPCQiTx_State_t WPCQiTx_ProcWaitID ( void )
{
	...
	if (qi_comm_message_received)
	{
		...
		if (qi_last_packet_header == WPCQi_IdetificationPacket_H)
		{
			...
			if (qi_major_version != 1 && qi_major_version != 2) 
			{
				PWM_vStopCoil();
	...
	{
		if (qi_packet_last_bit_timer_ms >= 105)
		{
		...
		else
		{
			...
			if (qi_packet_first_bit_timer_ms > 170)
			{
	...
	if (qi_comm_message_received)
	{
		...
		switch (qi_last_packet_header)
		{
			...
			case WPCQi_PowerContronHoldOffPacket_H:
				...
				if ((qi_holdoff_time_ms < 5) ||
				(qi_holdoff_time_ms > 205))
				{
					PWM_vStopCoil();
			...
			case WPCQi_ConfigurationPacket_H:
				...
				((qi_power_class != 0) && (qi_power_class != 1)) || (ic_max_power > 200))
				{
			...
			case WPCQi_ProprietaryOOBInfoPacket_H:
				...
				{	
					...
					{
					   sendMacAddToBle();
					   if (0) printf( "\r\nM=%x:%x:%x:%x:%x:%x,%d", qi_oobMacAddr[0], qi_oobMacAddr[1], qi_oobMacAddr[2], qi_oobMacAddr[3], qi_oobMacAddr[4], qi_oobMacAddr[5],commType);
#line 1267 "..\\WPCQiTx.c"
	...
	{
		if ((qi_packet_last_bit_timer_ms >= 105) && (received_holdoffcmd != 1)) 
		{
...
WPCQiTx_State_t WPCQiTx_ProcNegoResponse ( void )
{
	...
	if(qi_last_packet_header == WPCQi_FODStatusPacket_H) 
	{
#line 1394 "..\\WPCQiTx.c"
		if(counter_ms > 4)
		{
	...
	else if(qi_last_packet_header == WPCQi_GeneralRequestPacket_H) 
	{
		if(counter_ms > 4)
		{
	...
	else if(qi_last_packet_header == WPCQi_SpecificRequestPacket_H) 
	{ 
		if(counter_ms > 4)
		{
			counter_ms = 0;
			switch(qi_tx_last_packet_header)
			{
				...
				case WPCQi_SpecificRequest_GuaranteedPowerPacket_H: 
					...
					else 
					{
						...
						if (n_max_power != g_st_power_contract.guaranteeed_power) 
							g_st_power_contract.count |= 0x01;
						else 
							g_st_power_contract.count &= ~0x01;				
					}
					retState = QI_NEGO_WAIT_ID_S;
				...
				case WPCQi_SpecificRequest_FSKParameterPacket_H: 
					...
					if (qi_tx_received_power_header != g_st_power_contract.modulation_depth)
						g_st_power_contract.count |= 0x08;
					else
						g_st_power_contract.count &= ~0x08;
				...
				case WPCQi_SpecificRequest_MaxiumPowerPacket_H: 
					...
					if((qi_maximum_power_reserved != 0) || (max_power >= 200)) 
					{
						WPCQiTxFskLoopState = FSK_WAIT_S;
						WPCQiFskResponse = FSK_ND_RESPONSE;	
					}
					...
					{ 
						...
						{
							g_st_power_contract.count |= 0x02;
						} 
						else 
						{
							g_st_power_contract.count &= ~0x02;
...
WPCQiTx_State_t WPCQiTx_ProcPowerTransfer ( void )
{
	...
	control_error_pkt_ms += 1;
	received_power_pkt_ms += 1;
	modulation_error_pkt_ms += 1;

    if( commType == QI_COMMTYPE_COIL && control_error_pkt_ms == 800 && (qi_oob_flag == 0x01) )
    {
		...
		if (( (received_endNego != 1) && (qi_neg)) || ((commType == QI_COMMTYPE_COIL )&&(modulation_error_pkt_ms > 5000)))			
	    {
	        ...
	        received_endNego = 0;
	        if( commType == QI_COMMTYPE_COIL )if (1) printf( "\r\n ====> is not connected");
	        return retState;
	    }
		...
		if (qi_comm_message_received)
		{
			...
			switch (qi_last_packet_header)
			{
				...
				case WPCQi_ControlErrorPacet_H:
					...
					if(1)	
					{
						...
						if ( 1 )
						{
							...
							if( coil_sel_num == COIL_SEL_2)
							{
								PWM_vInitBuckbooster( 960, 960*190/200);
								WPCQi_InitVoltageHandleCEP(190);
							}
							else
							{
								PWM_vInitBuckbooster( 960, 960*170/200);
								WPCQi_InitVoltageHandleCEP(170);
							}
								
							if( coil_sel_num == COIL_SEL_2)
								PWM_vInitCoil(662, (662 >> 1 ));  
							else
								PWM_vInitCoil(pwmFreq, pwmFreq>>1);   
				...
				case WPCQi_24BitReceivedPowerPacket_H:
					received_power_pkt_ms = 0;
					t_received_power_ms = 24000;
					
					printf("\r\nADC val = %d\r\n", qi_received_power);
					if(qi_received_power > 880){
						printf("freq -> 145KHz\r\n");
						pwmFreq = 662;
					}else{
						printf("freq -> 110KHz\r\n");
					...
					if ( g_st_power_contract.max_power > 15 )
						temp = temp * 40 / 100 + 518;
					else
						temp = temp * 40 / 100 + 328;
					...
					if(qi_received_power_mode <= 0x02)
					{
						if((qi_control_error_value > 1) || 							(qi_control_error_value < -1))
					...
					else if(qi_received_power_mode == 0x04)
					{
				...
				case WPCQi_8BitReceivedPowerPacket_H:
					...
					{
						if(( ( qi_received_power_mode == 0x01 )||(qi_received_power_mode == 0x02))&&(qi_neg == 1))
						{
		...
		switch (control_loop_state)
		{
			...
			case FSK_WAIT_S:
				if (power_xfer_delay_ms >= qi_holdoff_time_ms)
				{
					...
					qi_coil_current_f16 = (float)(isense_counts() ) * (float)0.00322;
					...
					power_xfer_delay_ms += 1;
				break;

			case FSK_RUN_S:

				power_xfer_delay_ms += 1;
				if (power_xfer_delay_ms >= 30)
				{
					control_loop_state = FSK_IDLE_S;
				}
...
WPCQiTx_State_t WPCQiTx_ProcFODHandle ( WPCQiTx_State_t state )
{
	...
	switch( state )
	{
		...
		case QI_WAIT_PAD_REMOVAL_NO_TIMEOUT_S:
			...
			if (analog_ping_test(flash_config_ram.analog_ping_threshold) == 0)
			{
		...
		case QI_STATE_MACHINE_DELAY_S:
			...
			if (counter_ms == delay_counter)
			{
				...
				if (over_temp_flag && (rth_counts() > 91))
					over_temp_flag = 0;
			}
...
__attribute__ ((section(".ramfunc"))) void WPCQi_Tx(void)
{
...
#line 2364 "..\\WPCQiTx.c"
	...
	counter_ms += 1;
	qi_packet_first_bit_timer_ms += 1;

	
	if (!qi_receiving_start_bit_message)
		qi_packet_last_bit_timer_ms += 1;
}
