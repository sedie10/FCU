...
static fix16_t freq_prev_kHz_f16 = 110.000;           
static fix16_t voltage_prev_volt_f16 = 180;           




fix16_t min_freq_kHz_f16 = 110.000;    

fix16_t freq_cutoff = 145.000;
fix16_t duty_cutoff= 0.5;
static fix16_t duty_kp = 0.014;




static fix16_t volt_kp = 0.1;
#line 31 "..\\pid.c"

#line 52 "..\\pid.c"

uint8_t ot_limit_flag;                                

fix16_t P_f16, I_f16, PID_f16, freq_kHz_f16, scale_f16,ss;  
...
static __attribute__ ((section(".ramfunc"))) void set_pwm_from_virt_freq(fix16_t freq_kHz_f16,int Update_cep);
...
void WPCQi_InitFrequencyHandleCEP(void)
{
	...
	if (g_st_power_contract.max_power > 10)
    {
        freq_cutoff = 145.000;
        duty_cutoff = 0.5;
        duty_kp     = 0.014;
...
void WPCQi_InitHandleCEP(void)
{
  ...
  min_freq_kHz_f16 = 110.000;


  PWM_vStartBuckbooster();
  voltage_prev_volt_f16 = 192;		   
  PWM_vUpdateBuckbooster( 960, 960*voltage_prev_volt_f16/200);
...
__attribute__ ((section(".ramfunc"))) void WPCQi_HandleControlErrorPacket(int update_cep)
{
	static fix16_t coil_current_f16;                          
	fix16_t error_f16;                                       

	if((min_freq_kHz_f16 >= (fix16_t)110.000) && (qi_control_error_value == 0))
	{
	...
	{
		voltage_prev_volt_f16 = voltage_prev_volt_f16  ;
	}
	else
	...
	if( coil_sel_num == COIL_SEL_2)
	{
		if(voltage_prev_volt_f16 < (200 - 40 ) )
			voltage_prev_volt_f16 = (200 - 40 );
		else if(voltage_prev_volt_f16 > (200 - 0 ) )
			voltage_prev_volt_f16 = (200 - 0 );
	}
	else
	{
	if(voltage_prev_volt_f16 < (200 - 198 ) )
		voltage_prev_volt_f16 = (200 - 198 );
	else if(voltage_prev_volt_f16 > (200 - 8 ) )
		voltage_prev_volt_f16 = (200 - 8 );
	}
	dcdc_update_value = ((fix16_t)960) * voltage_prev_volt_f16 / (fix16_t)200;
	...
	PWM_vUpdateBuckbooster(960,dcdc_update_value_int );
#line 335 "..\\pid.c"
}

__attribute__ ((section(".ramfunc"))) void vdroop_control(int vin_counts)
{ 
...
__attribute__ ((section(".ramfunc"))) void set_pwm_from_virt_freq(fix16_t freq_kHz_f16,int update_cep)
...

{
  ...
  if (freq_kHz_f16 > (fix16_t)430.000)
    freq_kHz_f16 = 430.000;
  ...
  if (freq_kHz_f16 > (fix16_t)145.000)
  {
		...
		temp_f16 = freq_kHz_f16 - (fix16_t)145.000;

		freq_kHz_f16 = 145.000;

		
		

		temp_f16 = temp_f16 * duty_kp ;
		
		if (temp_f16 > (fix16_t)0.5 )
			temp_f16 = 0.5;

		temp_f16 = (fix16_t)0.5 - temp_f16;

		if (temp_f16 < (fix16_t)0.007) 
			temp_f16 = 0.007;
  ...
  if (freq_kHz_f16 > (fix16_t)145.000)
    act_kHz_f16 = 145.000;
  else if (freq_kHz_f16 < (fix16_t)110.000)
    act_kHz_f16 = 110.000;
  else
  ...
  temp_f16 = (fix16_t)(55 * 872);   
  temp_f16 = (fix16_t)(temp_f16 /act_kHz_f16 );  
  pwm_period_ticks = (int)((temp_f16 * (float)2 ) + 1);  

   
   
   pwm_duty_ticks = (int)((pwm_period_ticks * pwm_duty_scale)); 
 ...
 void ot_control(uint16_t rth_value)
{
  ...
  if (ot_limit_flag && (rth_value < (flash_config_ram.rth_ot_hysteresis_threshood + 63)))
  {
    if (ot_ctrl_count == 0)
    {
      if ((rth_value < prev_rth_value) && (state == QI_POWER_TRANSFER_S ) 
	  	 
	  	)
      {
        
        
        min_freq_kHz_f16 = (fix16_t)(freq_prev_kHz_f16 * (fix16_t)1.07); 
  ...
  else if (ot_limit_flag && (rth_value >= flash_config_ram.rth_ot_hysteresis_threshood + 30))  
  {
    ot_limit_flag = 0;
