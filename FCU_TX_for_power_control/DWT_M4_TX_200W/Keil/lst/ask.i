...
typedef enum
{
  ASKDECODE_PREAMBLE_HUNT_EDGE_S = 0, 
  ASKDECODE_DATA_HUNT_EDGE_S,         
...
typedef enum
{
  ...
  ASKBITDECODE_START_S, 
  ASKBITDECODE_DATA_S,    
  ASKBITDECODE_PARITY_S,  
  ASKBITDECODE_STOP_S,    
...
typedef enum
{
  ...
  ASKBYTEDECODE_START_S, 
  ASKBYTEDECODE_DATA_S,   
  ASKBYTEDECODE_CHECKSUM_S, 

} WPCQiTX_AskByteDecodeState_t;


extern WPCQiTX_AskBitDecodeState_t askBitDecodeState;                      
extern WPCQiTX_AskByteDecodeState_t askByteDecodeState;                   
...
static WPCQiTX_AskDecodeState_t AskDecodeState = ASKDECODE_PREAMBLE_HUNT_EDGE_S;                 
static uint8_t qi_bit_num_consecutive_ones = 0;                               	 
static uint8_t qi_bit_num_half_ones = 0;                                      	 
static uint8_t qi_bit_num_unsure_zero = 0;                                       
static WPCQiTX_AskBitDecodeState_t askBitDecodeState = ASKBITDECODE_START_S;                   
static uint8_t qi_byte_message_byte_count = 0;                                	 
static uint8_t qi_byte_bit_position = 0;                                      	 
static uint8_t qi_byte_num_ones = 0;                                          	 
static uint8_t qi_byte_rx_byte = 0;                                       		 
static uint8_t temp_qi_byte_rx_byte = 0;																			 
int num_comm_errors = 0;                                                  		 
int num_start_bit = 0; 
...
uint8_t qi_signal_strength = 0;                    
signed char qi_control_error_value = 0;                   
uint32_t qi_received_power = 0;                     
uint8_t qi_charge_status = 0;                      
uint8_t qi_holdoff_time_ms = 5;       
uint8_t qi_power_class = 1;                        
uint8_t qi_maximum_power_reserved = 0;
uint8_t qi_maximum_power = 3;                      
uint8_t qi_prop = 0;                               
...
static void ASK_vBitDecode(uint8_t BitValue);                        		 	 
static int ASK_vPacketDecode(void); 
__attribute__ ((section(".ramfunc"))) void ASK_vByteDecode(uint8_t Byte_Data);     						 
extern void qi_comm_message_state_machine_reset(void);                     
	...
	switch (AskDecodeState)
	{
			...
			{       
	      		...
	      		if ((period > 16080) && (period <= 31920)) 
				{
					...
					if ( period < 20400 )
					{
						...
						period_delta = period - 12000;
					}
					else

					{
					    if (qi_bit_num_consecutive_ones >= 4)
					    {
							
							AskDecodeState = ASKDECODE_DATA_HUNT_EDGE_S;         

							period_delta = period - 24000;
							...
							if(( qi_bit_num_unsure_zero == 1 ) &&( ( period + period_delta ) > 31920))
							{
								qi_bit_num_unsure_zero = 0;
								ASK_vBitDecode(0); 
							}
			      ...
			      else if ((period < 8040) && (( qi_bit_num_unsure_zero == 1 )))
			      {
				     qi_bit_num_unsure_zero = 0;
	              ...
	              else if ((period > 8040) && (period <= 15960)) 
	              { 
        ...
        case ASKDECODE_DATA_HUNT_EDGE_S:
			...
			if ( ( period <= 8040 ) )
			{
				...
				{
					if (((( period_delta + period ) - ( 8040) > 0) ) && (qi_bit_num_half_ones == 0))
					{
					...
					else if ((( period_delta + period ) > 8040 ) && (qi_bit_num_half_ones == 1))
					{
						qi_bit_num_half_ones = 0;									
						qi_bit_num_unsure_zero = 0;
						ASK_vBitDecode(1);
					...
					if ((( period_delta + period ) > 8040 ) && (qi_bit_num_unsure_zero == 1))
					{
						qi_bit_num_unsure_zero = 0;
						qi_bit_num_half_ones = 0;									
						ASK_vBitDecode(1);
				...
				if ((period > 8040 ) && (period <= 15960)) 
				{
		        
					if( period_delta < (-12000) )
					{
		      ...
		      else if((period > 16080) && (period <= 31920))
		      {
		
				if (period_delta < (-12000))
				{
					prev_period_delta = period_delta;
					period_delta = period + period_delta - 24000;
					qi_bit_num_unsure_zero = 1;	
					return;
				}
				else if ((period_delta < ( 24000 - 20400) ) && (qi_bit_num_unsure_zero == 1))
				{
					prev_period_delta = period_delta;
					period_delta = period + period_delta - 12000;
					qi_bit_num_unsure_zero = 0;
					ASK_vBitDecode(0);
					if( period_delta < 0 )
						qi_bit_num_half_ones = 1;	
					...
					{
						period_delta = period_delta - 12000;
						if( (period_delta > -7200)  && (period_delta  < 7200))
						{
							period_delta = 0;
							ASK_vBitDecode(0);
						}
				...
				period_delta = period - 24000;
				if ( ( period > 20400) && (qi_bit_num_half_ones == 1))
				{
					prev_period_delta = period_delta;
					period_delta = period - period_delta -24000 - 12000;
					qi_bit_num_half_ones = 0 ;
				...
				else if ( ( period < 20400 ) && (qi_bit_num_half_ones == 0))
				{
					qi_bit_num_unsure_zero = 1;		
					period_delta = period - 24000;
					return;
				}
				else if ( ( period < 20400 ) && (qi_bit_num_half_ones == 1))
				{
					prev_period_delta = period_delta;
					period_delta = period - 12000;
					qi_bit_num_half_ones = 0 ;
			  ...
			  else if ( ( period > 31920) && ( period < ( 24000 + 15960) ))
			  {
					prev_period_delta = period_delta;
					if(qi_bit_num_half_ones == 1 )
					{
...
__attribute__ ((section(".ramfunc"))) void ASK_vBitDecode( uint8_t BitValue)
{
		...
		case ASKBITDECODE_START_S:
			...
			else
			{
  			++num_comm_errors;
				...
				{
					CLK_SysTickDelay(15);
					if (0) printf( "  %d : 0x%x",jj,qi_rx_data_buf[jj]);
				}
...
#line 634 "..\\Ask.c"
		...
		case ASKBITDECODE_STOP_S:
			...
			{
				if (qi_byte_message_byte_count == 0) 
				{
					
				
					if (qi_byte_rx_byte <= 0x1F)                  
						num_message_bytes = 1;
					else if (qi_byte_rx_byte <= 0x7F)             
						num_message_bytes = 2 + ((qi_byte_rx_byte - 32) >> 4);
					else if (qi_byte_rx_byte <= 0xDF)             
				...
				else
				{
					...
					if (qi_byte_message_byte_count == num_message_bytes) 
					{
...
{ 
	...
	switch (askByteDecodeState)
	{
		...
		case ASKBYTEDECODE_DATA_S:
			...
			if (rx_message_byte_count == (qi_message_length - 1)) 
...
static __inline int ASK_vPacketDecode(void)
{
  int i;
  ...
  case WPCQi_ConfigurationPacket_H:
    ...
    qi_maximum_power = qi_rx_buffer[0] & 0x3F; 
  ...
  case WPCQi_24BitReceivedPowerPacket_H:
    ...
    qi_received_power = qi_rx_buffer[1]; 
    qi_received_power <<= 8;
  ...
  case WPCQi_SpecificRequestPacket_H: 
      ...
      else if(qi_rx_buffer[0] == WPCQi_SpecificRequest_MaxiumPowerPacket_H)
      {
          ...
          qi_maximum_power = qi_rx_buffer[1] & 0x3F; 
      }
