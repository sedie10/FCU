...
void sendConfigurationToBle( void )
{
	...
	txBuf[2] = (qi_guaranteed_class << 6) | 0x28;
	txBuf[3] = (qi_potential_class << 6) | (0x28);
	txBuf[4] = 0;
	len = 5;
...
void UART0_Proc( uint8_t data )
{
	...
	switch( data )
	{
	   ...
	   case 0x7D:
			 ...
			 if( uart_byte_count > 128 ) uart_byte_count = 0;   
			 uartCmdReading = 1;
	   	   }
		   else
		   {
				...
				case WPCQi_SpecificRequestPacket_H:					
					...
					switch(negSpecCmd)
					{
						...
						case WPCQi_SpecificRequest_MaxiumPowerPacket_H:
							...
							qi_maximum_power = uart_buffer[3] & 0x3F; 
...
uint8_t getString_UART0( void )
{
  ...
  if(++q_UARTBLE_rd >= 256 ) q_UARTBLE_rd = 0;
...
void UART0_IRQHandler(void)
{
	...
	uint32_t u32IntSts = ((UART_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x30000UL))->INTSTS;

    if((u32IntSts & (0x1ul << (8))) || (u32IntSts & (0x1ul << (12))))
    {
    
	    buf_UARTBLE[q_UARTBLE_wr] = ((((UART_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x30000UL)))->DAT); 
		if( ++q_UARTBLE_wr >= 256 ) q_UARTBLE_wr = 0;
    }
}
