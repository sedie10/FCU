...
void WPCQiTx_FskSendConfiguration(void)
{
	...
	g_qi_tx_buffer[1] = (0x01 << 6) | 0x28;
	g_qi_tx_buffer[2] = (0x01 << 6) | (0x28);
	g_qi_tx_buffer[3] = 0; 
	g_qi_tx_buffer[4] = g_qi_tx_buffer[0] ^ g_qi_tx_buffer[1] ^ g_qi_tx_buffer[2] ^ g_qi_tx_buffer[3] ;
	g_qi_tx_size = 5;
...
void WPCQiTx_TxState(void)
{
	switch (WPCQiTxFskLoopState)
	{
		...
		case FSK_WAIT_S:
			...
			if (qi_packet_last_bit_timer_ms >= 4)
			{
				...
				{
                    if( commType == QI_COMMTYPE_BLE &&  qi_oob_flag == 0x01 ) sendResponseToBle(0xFF);
					else	WPCQiTx_FskSendAck();
					if (0) printf( "\r\nACK");
				}
				else if (WPCQiFskResponse == FSK_NAK_RESPONSE)
				{
                    if( commType == QI_COMMTYPE_BLE &&  qi_oob_flag == 0x01 ) sendResponseToBle(0x00);
					else WPCQiTx_FskSendNack();
					if (0) printf( "\r\nNAK");
				}
				else if (WPCQiFskResponse == FSK_ND_RESPONSE)
				{
                    if( commType == QI_COMMTYPE_BLE &&  qi_oob_flag == 0x01 ) sendResponseToBle(0x55);
		...
		case FSK_RUN_S:
			if (qi_packet_last_bit_timer_ms >= 30)
			{
