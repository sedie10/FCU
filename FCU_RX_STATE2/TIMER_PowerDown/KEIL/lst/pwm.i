...
#line 13 "..\\..\\..\\..\\Library\\StdDriver\\src\\pwm.c"
                                  ...
                                  uint32_t u32CaptureEdge)
{
    ...
    uint32_t u32PWMClkTbl[8] = {(12000000UL), (32768UL), 0, (22118400UL), 0, 0, 0, (10000UL)};
    uint32_t u32NearestUnitTimeNsec;
    uint8_t  u8Divider = 1;
     
    uint32_t u32PWMDividerToRegTbl[17] = {0, 4, 0, 0, 1, 0, 0, 0, 2, 0, 0, 0, 0, 0, 0, 0, 3};
    uint16_t u16Prescale = 2;
    uint16_t u16CNR = 0xFFFF;

    if(pwm == ((PWM_T *) ((( uint32_t)0x40000000) + 0x40000)))
    {
        if(u32ChannelNum < 2) 
            u32Src = ((((CLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL2 & ((1ul << 8))) >> (8 - 2)) | (((CLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL1 & ((3ul << 28))) >> (28);
        else  
            u32Src = ((((CLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL2 & ((1ul << 9))) >> (9 - 2)) | (((CLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL1 & ((3ul << 30))) >> (30);
    }
    else  
    {
        if(u32ChannelNum < 2) 
            u32Src = ((((CLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL2 & ((1ul << 10))) >> (10 - 2)) | (((CLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL2 & ((3ul << 4))) >> (4);
        else  
            return 0;
    }
    ...
    for(; u16Prescale <= 0x100; u16Prescale++)
    {
        ...
        {
            if((u16Prescale == 0x100) && (u8Divider == 16))  
                break;
    ...
    (pwm)->PPR = ((pwm)->PPR & ~((0xFFul << 0) << ((u32ChannelNum >> 1) * 8))) | (u16Prescale << ((u32ChannelNum >> 1) * 8));
    (pwm)->CSR = ((pwm)->CSR & ~((7ul << 0) << (4 * u32ChannelNum))) | (u8Divider << (4 * u32ChannelNum));
    
    (pwm)->PCR &= ~((1ul << 30) << (u32ChannelNum >> 1));
    (pwm)->PCR |= (1ul << 3) << (8 * u32ChannelNum);
    *((volatile uint32_t *)((((uint32_t) & ((pwm)->CNR0)) + (u32ChannelNum) * 12))) = u16CNR;
                                 ...
                                 uint32_t u32DutyCycle)
{
    ...
    (pwm)->PPR = ((pwm)->PPR & ~((0xFFul << 0) << ((u32ChannelNum >> 1) * 8))) | (u8Prescale << ((u32ChannelNum >> 1) * 8));
    (pwm)->CSR = ((pwm)->CSR & ~((7ul << 0) << (4 * u32ChannelNum))) | (u8Divider << (4 * u32ChannelNum));
    ...
    if(u32DutyCycle)
    {
        *((volatile uint32_t *)((((uint32_t) & ((pwm)->CMR0)) + u32ChannelNum * 12))) = u32DutyCycle * (u16CNR + 1) / 100 - 1;
    }
    else
    {
        *((volatile uint32_t *)((((uint32_t) & ((pwm)->CMR0)) + u32ChannelNum * 12))) = 0;
    }
    *((volatile uint32_t *)((((uint32_t) & ((pwm)->CNR0)) + (u32ChannelNum) * 12))) = u16CNR;
...
void PWM_Stop(PWM_T *pwm, uint32_t u32ChannelMask)
{
    ...
    for(i = 0; i < (4); i ++)
    {
        if(u32ChannelMask & (1 << i))
        {
            *((volatile uint32_t *)((((uint32_t) & ((pwm)->CNR0)) + i * 12))) = 0;
        }
    }
}
...
void PWM_DisableADCTrigger(PWM_T *pwm, uint32_t u32ChannelNum)
{
    (pwm)->TCON = ((pwm)->TCON & ~((1ul << 0) << u32ChannelNum));
}
...
void PWM_ClearADCTriggerFlag(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Condition)
{
    (pwm)->TSTATUS = ((1ul << 0) << u32ChannelNum);
}
...
uint32_t PWM_GetADCTriggerFlag(PWM_T *pwm, uint32_t u32ChannelNum)
{
    return (((pwm)->TSTATUS & ((1ul << 0) << (u32ChannelNum))) ? 1 : 0);
}
...
void PWM_EnableDeadZone(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Duration)
{
    ...
    (pwm)->PPR = ((pwm)->PPR & ~((0xFFul << 16) << (8 * u32ChannelNum))) | (u32Duration << (16 + 8 * u32ChannelNum));
    
    (pwm)->PCR |= ((1ul << 4) << u32ChannelNum);
}
...
void PWM_ClearCaptureIntFlag(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Edge)
{
    ...
    if(u32ChannelNum < 2)
        (pwm)->CCR0 = ((pwm)->CCR0 & 0x000F000F) | ((1ul << 4) << (u32ChannelNum * 16)) | (u32Edge << (u32ChannelNum * 16 + 5));
    else
        (pwm)->CCR2 = ((pwm)->CCR2 & 0x000F000F) | ((1ul << 4) << ((u32ChannelNum - 2) * 16)) | (u32Edge << ((u32ChannelNum - 2) * 16 + 5));
}
...
uint32_t PWM_GetCaptureIntFlag(PWM_T *pwm, uint32_t u32ChannelNum)
{
    if(u32ChannelNum < 2)
    {
        return (((pwm)->CCR0 & (((1ul << 6) | (1ul << 7)) << (u32ChannelNum * 16))) >> (6 + u32ChannelNum * 16));
    }
    else
    {
        return (((pwm)->CCR2 & (((1ul << 6) | (1ul << 7)) << ((u32ChannelNum - 2) * 16))) >> (6 + (u32ChannelNum - 2) * 16));
    }
...
uint32_t PWM_GetDutyIntFlag(PWM_T *pwm, uint32_t u32ChannelNum)
{
    return (((pwm)->PIIR & ((1ul << 8) << u32ChannelNum)) ? 1 : 0);
}
...
void PWM_EnablePeriodInt(PWM_T *pwm, uint32_t u32ChannelNum,  uint32_t u32IntPeriodType)
{
    (pwm)->PIER = ((pwm)->PIER & ~((1ul << 16) << (u32ChannelNum >> 1))) |                   ((1ul << 0) << u32ChannelNum) | (u32IntPeriodType << (u32ChannelNum >> 1));
...
uint32_t PWM_GetPeriodIntFlag(PWM_T *pwm, uint32_t u32ChannelNum)
{
    return (((pwm)->PIIR & ((1ul << 0) << (u32ChannelNum))) ? 1 : 0);
}
